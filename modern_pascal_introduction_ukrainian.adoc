include::locale/attributes-uk.adoc[]
...
## Вступ: Для чого це потрібно?

У світі існує безліч книг про Pascal, однак, багато з них говорять про "застарілий" Pascal, без класів, модулів, generic-ів.
// Деякі з них розповідають про Pascal ще до того, як у нього були класи. Деякі з них показують класи, як це зроблено в Delphi, але не згадують про сучасніші функції, як-от дженерики.

Тож я написав цей короткий вступ до того, що я називаю *сучасний Object Pascal*. Більшість програмістів, які його використовують, насправді не називають його _"сучасний Object Pascal"_, ми просто називаємо його _"наш Pascal"_. Але, представляючи мову, я вважаю важливим підкреслити, що це сучасна, об’єктно-орієнтована мова. Він *суттєво* еволюціонував порівняно зі старим (Turbo) Pascal, який багато людей вивчали колись в школах. По своїх можливостях він дуже схожий на C++, Java чи C#.

* Pascal має всі сучасні особливості, які можна очікувати - класи, модульну структуру, інтерфейси, generic-и...
* Pascal компілюється в швидкий машинний код.
* Pascal є типобезпечною мовою, що в деяких випадках суттєво спрощує відладку.
* Взагалі, Pascal - високорівнева мова, проте він має можливість використовувати низькорівневі підходи, якщо це необхідно.

Він також має чудовий кросплатформовий компілятор з відкритим вихідним кодом: _Free Pascal Compiler_ (http://freepascal.org/). Для нього існує декілька IDE (які включають редактор коду, відлагоджувач, бібліотеки компонентів, дизайнер форм), одна з найвідоміших називається _Lazarus_ (http://lazarus.freepascal.org/). Автор цієї книги є творцем _Castle Game Engine_ (https://castle-engine.io/) - котрий є потужним дво- та тримірним ігровим рушієм, що використовує цю мову, для створення відеоігор під багато платформ: Windows, Linux, MacOSX, Android, iOS, Nintendo Switch; також очікується WebGL.

Цей вступ здебільшого спрямований на програмістів, які вже мають досвід роботи з іншими мовами. Ми не розглядатимемо значення деяких універсальних понять, таких як _"що таке клас"_, ми лише покажемо, як це зробити в Pascal.

_Прим.перекл. Тут і надалі ми будемо використовувати оригінальні англомовні терміни, такі як unit, constructor, override, reference-counting у випадках, якщо вони є ключовими словами мови або не мають загальноприйнятих перекладів на українську мову. Також часто надається перевага використанню оригінального англійського слова його транслітерації, як у випадку з поняттям "generic" вище._
...
[source,pascal]
----
include::code-samples_ukrainian/hello_world.lpr[]
----

Це повноцінна програма, яку можна _компілювати_ та _запускати_.

* Якщо Ви користуєтесь компілятором FPC з командного рядка, просто створіть новий файл `myprogram.lpr` з кодом програми всередині й виконайте команду `fpc myprogram.lpr`.
* Якщо Ви використовуєте _Lazarus_, створіть новий проект (меню _Проєкт_ -> _Новий проєкт_ -> _Проста програма_). Збережіть його як `myprogram` і вставте цей вихідний код як основний файл. Скомпілюйте за допомогою пункту меню _Виконання -> Компілювати_.
* Це програма командного рядка, тому в будь-якому випадку просто запустіть скомпільований виконуваний файл із командного рядка.

Решта цієї книги розповідає про мову Object Pascal, тому не очікуйте побачити щось більш привабливе, ніж речі командного рядка. Якщо Ви хочете побачити щось класне, просто створіть новий проект GUI у _Lazarus_ (_Проєкт_ -> _Новий проєкт_ -> _Застосунок_).
//Пограйтесь, скиньте кілька кнопок на форму, обробіть їхні події (наприклад, `OnClick`).
Вуаля — робоча програма з графічним інтерфейсом користувача, кросплатформна, з рідним виглядом усюди, використовуючи зручну бібліотеку візуальних компонентів. _Lazarus_ і _Free Pascal Compiler_ постачаються з великою кількістю готових модулів для роботи в мережі, GUI, бази даних, форматів файлів (XML, json, зображення...), потоків і всього іншого, що може знадобитися. Я вже згадував свій крутий _Castle Game Engine_ раніше :)
// Бібліотеки, створені іншими мовами (dll, so, dylib), також можна легко використовувати з FPC (і для більшості з них ви знайдете готові модулі «заголовків» і навіть модулі, які обертають їх у більш сучасний об’єктно-орієнтований API).

### Функції, порцедури, примітивні типи

[source,pascal]
----
include::code-samples_ukrainian/functions_primitives.lpr[]
----
...
begin
  Result := S + 'щось';
  Result := Result + ' щось більше!';
  Result := Result + ' і більше!';
end;
----
...
Зверніть увагу, що результат функції можна відхилити. Будь-яку функцію можна використовувати як процедуру. Це має сенс, якщо функція має якийсь _побічний ефект_ (наприклад, вона змінює глобальну змінну), крім обчислення результату. Наприклад:
...
begin
  Count := 10;
  CountMe; // результат функції відкидається, але функція виконується, Count тепер дорівнює 11
  MyCount := CountMe; // використовуйте результат функції, MyCount дорівнює Count, який тепер дорівнює 12
end.
----
...
### Логічні оператори, опертори відношення (порівняння) та побітові оператори

_Логічні оператори_ називаються `and`, `or`, `not`, `xor`. Їх сенс, мабуть, очевидний (пошукайте _"exclusive or"_ якщо Ви не впевннені що робить _xor_ :)). Вони приймають _логічні аргументи_, і повертають результат типу _boolean_. Вони також можуть виконувати роль _побітових операторів_ у випадку коли обидва аргументи - цілочисельні значення, у цьому випадку вони повертають ціле число.

Оператори _відношення (порівняння)_ наступні: `=`, `<>`, `>`, `<`, `\<=`, `>=`. Якщо ви звикли до C-подібних мов, зауважте, що в Pascal ви порівнюєте два значення (перевіряєте, чи вони рівні) за допомогою одного символу рівності `A = B` (на відміну від C, де ви використовуєте `A == B`). Спеціальним оператором _присвоєння_ в Pascal є `:=`.

_Логічні (або побітові) оператори мають вищий пріоритет, ніж оператори відношення_. Вам може знадобитися використовувати дужки навколо деяких виразів, щоб мати потрібний порядок обчислень.
...
var
  A, B: Integer;
begin
  if A = 0 and B <> 0 then ... // НЕПРАВИЛЬНИЙ приклад
----

Наведене вище не компілюється, тому що компілятор спочатку хоче виконати побітове `and` в середині виразу: `(0 and B)`. Це побітова операція, яка повертає ціле значення. Потім компілятор застосовує оператор `=`, який дає логічне значення `A = (0 and B)`. І, нарешті, помилка _"невідповідність типу"_ виникає після спроби порівняти логічне значення `A = (0 and B)` і ціле значення `0`.
...
Використовується _коротка оцінка_. Розглянемо такий вираз:
...
* Гарантується, що спершу буде обчислено функцію `MyFunction(X)`.
* І якщо `MyFunction(X)` поверне `false`, тоді значення виразу відоме (значення `false and що-завгодно` завжди є `false`), і `MyOtherFunction(Y)` не буде виконано взагалі.
* Аналогічне правило для виразу `або`. Там, якщо відомо, що вираз має значення `true` (оскільки 1-й операнд має значення `true`), другий операнд не обчислюється.
* Це особливо корисно під час написання таких виразів, як
+
[source,pascal]
----
...
----
+
Це буде нормально, навіть якщо `A` дорівнює `nil`. Ключове слово `nil` — це вказівник, що дорівнює нулю (коли представлено у вигляді числа). Це називається _нульовим вказівником_ у багатьох інших мовах програмування.

// * Використання `and` між двома логічними значеннями - логічне `and`, і результат теж буде логічним. Іншими словами, результат `true` лише якщо обидва операнди `true`, інкаше - `false`.

// * Використання `and` між двома цілочисельними значеннями - побітове `and`, і результат теж цілочисельний. Операнди конвертуються до однакового числа біт, і схоже правило виконується для кожного біта, встановлюючи кожен біт в `0` або `1`. Якщо це виконується над потенційно негативними числами, потрібно розуміти як негативні числа кодуються в пам'яті (_"доповнення до двох"_).

### Перевірка одного виразу для кількох значень (case)
...
Пункт `else` є необов’язковим (і відповідає `default` у C-подібних мовах). Якщо жодна умова не відповідає, і немає `else`, нічого не відбувається.

Якщо Ви прийшли з C-подібних мов, і порівняєте це з подібною інструкцією `switch` у цих мовах, Ви помітите, що немає автоматичного _переходу на наступну інструкцію_. Це свідоме благословення в Паскалі. Вам не потрібно пам’ятати про розміщення інструкцій `break` після кожного блоку. Під час кожного виконання виконується _щонайбільше одна_ гілка `case`, і все.

### Перечислювані та порядкові типи, множини та масиви постійної довжини

Перечислюваний тип в Pascal — дуже гарний, прозорий тип. Ймовірно, Ви будете ним користуватись набагато частіше, ніж перечислюваними типами в інших мовах:)
...
Згідно з домовленістю до імен enum ставиться дволітерне скорочення імені типу, отже `ak` = скорочення для _"Animal Kind"_. Це корисна домовленість, оскільки імена enum знаходяться в просторі імен модуля (глобальному). Таким чином, додавши до них префікс `ak`, ви зменшуєте ймовірність конфліктів з іншими ідентифікаторами.

NOTE: Конфлікти в іменах не перешкода. Це нормально, якщо різні модулі визначають той самий ідентифікатор. Але було б гарною ідеєю все одно намагатися уникнути конфліктів, щоб код був простим для розуміння та аналізу.

NOTE: Ви можете уникнути розміщення імен перечислюваних типів у глобальному просторі імен за допомогою директиви компілятора `{$scopedenums on}`. Це означає, що Вам доведеться отримати до них доступ, попередньо зазначивши назву типу, як-от `TAnimalKind.akDuck`. У цій ситуації потреба в префіксі `ak` зникає, і ви, ймовірно, просто будете залишати назви `Duck, Cat, Dog`. Це схоже на те, як працюють перечислювані списки в C#.

Той факт, що перерахований тип є _прозорим_, означає, що його не можна напряму призначити цілочисельним величинам. Проте, для потреби такого використання Ви можете використати `Ord(MyAnimalKind)` для примусового перетворення перечислюваного типу на цілочислений або приведення типу `TAnimalKind(MyInteger)` для примусового перетворення цілого числа у відповідний перечислюваний тип. В останньому випадку спочатку переконайтеся, що `MyInteger` знаходиться в належному діапазоні (від 0 до `Ord(High(TAnimalKind))`).

Перечислювані та порядкові типи можуть використовуватися як індекси масиву:
...
Вони також можуть бути використані для створення наборів (вони ж множини, або внутрішні бітові поля):
...
[source,pascal]
----
include::code-samples_ukrainian/loops.lpr[]
----

*Про цикли `repeat` і `while`*:

Є дві відмінності між цими типами циклів:

1. Умова циклу має протилежне значення. В циклі `while .. do` Ви вказуєте _коли продовжувати_, але в циклі `repeat .. until` ви говорите йому _коли зупинитись_.
2. У випадку з циклом `repeat`, _умова не перевіряється на початку виконання_. Тож цикл `repeat` виконується завжди хоча б один раз.

*Про цикли `for I := ...`*:

Конструкція `for I := .. to .. do ...` схожа на C-подібний цикл `for`. Проте, вона більш обмежена, оскільки неможливо вказати довільну дію/умову для контролю виконання ітерацій циклу. Ця конструкція використовується строго для ітерацій послідовних цілих чисел (або інших порядкових типів). Єдина доступна гнучкість полягає в тому, що можна використовувати  `downto` замість `to`, щоб рахувати в оберненому порядку.

З іншого боку, він виглядає зрозумілим і дуже оптимізованим у виконанні. Зокрема, _вирази для нижньої та верхньої межі обчислюються лише один раз_, перед початком циклу.

Зауважте, що значення змінної лічильника циклу (`I` в цьому прикладі) може бути _невизначеним_ після закінчення циклу через модливі оптимізації. Доступ до значення `I` після завершення циклу може викликати попередження компілятора. _Якщо_ Ви не вийшли завчасно з циклу за допомогою `Break` або `Exit`: в цьому випадку змінна лічильника ґарантовано збереже останнє значення.

*Про цикли `for I in ...`*:

Конструкція `for I in .. do ..` подібна до конструкції `foreach` в багатьох сучасних мовах програмування. Вона розумно працює з багатьма вбудованими типами:

* Може перебирати всі значення масиву (приклад више).
* Може перебирати всі можливі значення перечислюваного типу:
+
[source,pascal]
----
var
...
----
* Може перебирати всі елементи набору (множини):
+
[source,pascal]
----
var
...
----
* І також працює на всіх типах, визначених користувачем, в т.ч. generic-и, напр. `TObjectList` чи `TFPGObjectList`.
+
[source,pascal]
----
include::code-samples_ukrainian/for_in_list.lpr[]
----
+
Ми ще не пояснювали концепцію класів, тому останній приклад може бути ще не очевидним - просто продовжуйте, це матиме сенс пізніше :)

### Вивід, лоґування

Щоб просто виводити рядки в Pascal, скористайтеся процедурою `Write` або `WriteLn`. Остання автоматично додає новий рядок у кінці.

Це «чарівна» процедура в Pascal. Вона приймає змінну кількість аргументів, і вони можуть мати будь-який тип. Під час відображення всі вони перетворюються на рядки зі спеціальним синтаксисом для визначення заповнення та точності чисел.

[source,pascal]
----
WriteLn('Привіт, світ!');
WriteLn('Ви можете виводити цілі числа: ', 3 * 4);
WriteLn('Ви можете його відфоматувати: ', 666:10);
WriteLn('Також можна вивести число з плаваючою крапкою: ', Pi:1:4);
----

Щоб явно використати перехід на новий рядок, використовуйте константу `LineEnding` (з бібліотеки FPC RTL). (_Castle Game Engine_ містить коротшу константу `NL`.) Рядки Pascal не інтерпретують жодної спеціальної послідовності, що починається з оберненої косої (`/`), тож наступний код

[source,pascal]
----
WriteLn('Один рядок.\nДругий рядок.'); // НЕПРАВИЛЬНИЙ приклад
----

не спрацює так, як очікувалось. Ось це працює коректно:

[source,pascal]
----
WriteLn('Один рядок.' + LineEnding + 'Другий рядок.');
----
...
----
WriteLn('Один рядок.');
WriteLn('Другий рядок.');
----

Зауважте, що це спрацює лише в _консольних_ додатках. Переконайтесь, що зазначено директиву компілятора `{$apptype CONSOLE}` (але *не* `{$apptype GUI}`) в головному файлі програми. В деяких операційних системах це не має значення і завжди працює (Unix), але в інших операційних системах спроба шось вивести з графічного додатку викличе помилку (Windows).

*В Castle Game Engine:* для виведення відлагоджувальної інформації користуйтесь `WriteLnLog` чи `WriteLnWarning`, але не рекомендується використовувати `WriteLn`. Вони завжди виконують вивід в корисному напрямі. В Unix, це стандартний вивід в консоль, в Windows-додатках з графічним інтерфейсом - лоґ-файл, в Android вивід буде направлено в _Android logging facility_ (видимий за допомогою команди `adb logcat`). Використання `WriteLn` має обмежену кількість випадків, наприклад, якщо розробляється застосунок командного рядка (як в конвертері/генераторі 3D-моделей) і точно відомо, що доступний _стандартний вивід_.

### Перетворення даних в рядковий тип
...
[source,pascal]
----
include::code-samples_ukrainian/myunit.pas[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/myunit_test.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/initialization_finalization.pas[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/anotherunit.pas[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/inheritance.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/is_as.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/exception_finally.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/method_calls_inheritance_1.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/method_calls_inherited.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/without_virtual_methods.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/with_virtual_methods.lpr[]
----
...
I advise using `FreeAndNil(A)` always, without exceptions, and never to call directly the `Free` method or `Destroy` destructor. The _Castle Game Engine_ does it like that. It helps to keep a nice assertion that _all references are either nil, or point to valid instances_.
...
[source,pascal]
----
include::code-samples_ukrainian/file_stream.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_lists.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_sorting.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_dictionary.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_object_dictionary.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/persistent.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/callbacks.lpr[]
----
* A method: declare with `of object` at the end.
+
[source,pascal]
----
include::code-samples_ukrainian/callbacks_of_object.lpr[]
----
+
...
[source,pascal]
----
include::code-samples_ukrainian/generics.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generic_functions.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/records.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading_classes.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading_records.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading_records_lists.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/static_class_method.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/class_properties.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/exception_in_constructor_test.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interfaces_corba_test.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/gen_guid.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interfaces_com_with_ref_counting.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interfaces_com_test.lpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interface_casting.lpr[]
----
